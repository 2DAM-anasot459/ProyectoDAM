...
public partial class CrearLocalizacion : ContentPage, IQueryAttributable
{
	private readonly CrearLocalizacionViewModel vm;
	...
	public CrearLocalizacion(CrearLocalizacionViewModel viewModel)
	{
        InitializeComponent();
	...
	//Con este metodo podemos recibir el id del activo que hemos seleccionado en la pantalla de gestion
    ...
    private void OnMapaTapped(object sender, TappedEventArgs e)
	{
		try
		{
			...
			//En esta operacion realizamos cuatro accciones
			//Quitamos el centrado de la imagen
			//Quiramos el desplazamiento por la imagen
			//Quitamos el zoom
			//Volver a colocar el origen en la esquina superior izquierda
			...
			//Controlamos que al tocar fuera del margen del mapa no realiza ninguna acción
			if (contenidoX < 0 || contenidoY < 0 || contenidoX > baseW || contenidoY > baseH) return;

           //Calculamos las coordenadas en valores de 0 a 1 ya que usamos una imagen
